...
import { deleteObject, deleteGroup, deleteLight } from '../services/firestoreService';
...
export const useSceneStore = create<SceneState>((set, get) => ({
  ...
  removeObject: (id) =>
    set((state) => {
      ...
      // Delete from database if we have project context
      if (state.currentProjectId) {
        deleteObject(id, state.currentProjectId).catch(error => {
  ...
  removeGroup: (groupId) =>
    set((state) => {
      ...
      // Delete from database if we have project context
      if (state.currentProjectId) {
        deleteGroup(groupId, state.currentProjectId).catch(error => {
  ...
  removeLight: (lightId) =>
    set((state) => {
      ...
      // Delete from database if we have project context
      if (state.currentProjectId) {
        deleteLight(lightId, state.currentProjectId).catch(error => {
    ...
    set((state) => {
      ...
      if (state.currentProjectId !== projectId) {
        return {
          ...
          selectedObject: null,
          selectedLight: null,
          isLoading: !!projectId, // Set loading if we have a project to load
          hasUnsavedChanges: false,
          lastSaved: null
        };
      }
  ...
  loadProjectData: async () => {
    ...
    try {
      ...
      const { getObjects, getGroups, getLights } = await import('../services/firestoreService');
      const { firestoreToObject } = await import('../services/firestoreService');

      // Load all project data
      const [firestoreObjects, firestoreGroups, firestoreLights] = await Promise.all([
        getObjects(state.currentUserId, state.currentProjectId),
        getGroups(state.currentUserId, state.currentProjectId),
        getLights(state.currentUserId, state.currentProjectId)
      ]);
      ...
      const lights = firestoreLights.map(firestoreLight => ({
        ...firestoreLight,
      ...
      set({
        objects,
        groups: firestoreGroups.map(g => ({ ...g, id: g.id! })),
        lights,
        isLoading: false,
        hasUnsavedChanges: false,
        lastSaved: new Date()
      ...
      console.log(`Loaded project data: ${objects.length} objects, ${firestoreGroups.length} groups, ${lights.length} lights`);
    } catch (error) {
  ...
  saveProjectData: async () => {
    ...
    if (!state.currentProjectId || !state.currentUserId) {
      throw new Error('No project context for saving');
    }

    try {
      // Import the service functions dynamically
      const { saveObject, saveGroup, saveLight, objectToFirestore } = await import('../services/firestoreService');
      ...
      const objectPromises = state.objects.map(async (obj) => {
        ...
        return await saveObject(firestoreData, state.currentUserId!, state.currentProjectId!);
      });
      ...
      const groupPromises = state.groups.map(async (group) => {
        ...
        return await saveGroup(firestoreGroup, state.currentUserId!, state.currentProjectId!);
      });

      // Save all lights
      const lightPromises = state.lights.map(async (light) => {
        const firestoreLight = {
          name: light.name,
          type: light.type,
        ...
        return await saveLight(firestoreLight, state.currentUserId!, state.currentProjectId!);
      });
      ...
      console.log(`Saved project data: ${state.objects.length} objects, ${state.groups.length} groups, ${state.lights.length} lights`);
    } catch (error) {
